...
export class FrameImpl implements IFrame {
  ...
  get body(): string {
    if (!this._body && this.isBinaryBody) {
      this._body = new UTF8TextDecoder().decode(this._binaryBody);
    }
    return this._body;
  }
  ...
  get binaryBody(): Uint8Array {
    if (!this._binaryBody && !this.isBinaryBody) {
      this._binaryBody = new UTF8TextEncoder().encode(this._body);
    }
  ...
  private static toUnit8Array(
    ...
    const uint8CmdAndHeaders = new UTF8TextEncoder().encode(cmdAndHeaders);
