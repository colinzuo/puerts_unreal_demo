...
class Client {
     ...
     * ```
     */
    stompVersions = versions_1.Versions.default;
    /**
     ...
     */
    get active() {
        return this.state === types_1.ActivationState.ACTIVE;
    }
    /**
     ...
     * When deactivating it may go from ACTIVE to INACTIVE without entering DEACTIVATING.
     */
    state = types_1.ActivationState.INACTIVE;
    _reconnector;
    /**
     * Create an instance.
     */
     ...
     */
    activate() {
        if (this.state === types_1.ActivationState.DEACTIVATING) {
        ...
        this._changeState(types_1.ActivationState.ACTIVE);
        this._connect();
    }
    async _connect() {
        if (this.connected) {
        ...
        this._stompHandler = new stomp_handler_1.StompHandler(this, webSocket, {
            debug: this.debug,
            ...
            onWebSocketClose: evt => {
                ...
                if (this.state === types_1.ActivationState.DEACTIVATING) {
                    ...
                    this._changeState(types_1.ActivationState.INACTIVE);
                }
    ...
    async deactivate() {
        let retPromise;
        if (this.state !== types_1.ActivationState.ACTIVE) {
            this.debug(`Already ${types_1.ActivationState[this.state]}, ignoring call to deactivate`);
            return Promise.resolve();
        }
        this._changeState(types_1.ActivationState.DEACTIVATING);
        // Clear if a reconnection was scheduled
        if (this._reconnector) {
            clearTimeout(this._reconnector);
        }
        if (this._stompHandler &&
            this.webSocket.readyState !== types_1.StompSocketState.CLOSED) {
